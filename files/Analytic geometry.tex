...
In most cases, however, $m \ne 0$ and $m \in (-\infty, \infty)$, and both $x$ and $y$ are real numbers.  That is, $m$ is usually not infinite but also non-zero.
...
Notice that the equation of a line is determined by both the slope and one point, for example the $y$-intercept.  One can draw a whole family of parallel lines with the same slope and different $y$-intercepts.  Here are three lines $y = 2x + y_0$ for $y_0 = \{ 0, 1, 2 \}$.
...
Probably the most common thing to do with a quadratic equation like this is to find the roots, the values of $x$ for which $y=0$ is a solution.  These are the points where the graph of the curve crosses the $x$-axis.  (It may not do so, it is possible to have 0, 1 or 2 roots).
...
but roots do not have to be integers (or even rational), and a more productive approach is to find a general formula by the process of \emph{completing the square}.  First, multiply through by $1/a$ and rearrange:
